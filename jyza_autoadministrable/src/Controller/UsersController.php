    ...
    public function login()
    {
        ...
        if ($this->request->is('post')) {
            ...
            if (!$user) {
                ...
                // Fallback manual: verificar hash localmente y crear identidad si coincide
                $hasher = new DefaultPasswordHasher();
                if ($hasher->check($data['password'], $user->password)) {
                    // Forzar identidad en el servicio de autenticación
                    $this->Authentication->setIdentity($user);
                    // Redirigir según rol/estado (igual que el flujo válido)
                    if ($user->get('estado') === 'inactivo' || $user->get('estado') === 'deshabilitado') {
                        $this->Authentication->logout();
                        $this->Flash->error(__('Su usuario está inactivo o deshabilitado. Por favor, comuníquese con un administrador.'));
                        return $this->redirect(['action' => 'login']);
                    }
                    if ((int)$user->get('rol') === 1) {
                        return $this->redirect(['action' => 'index']);
                    }
