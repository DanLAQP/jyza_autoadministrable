...
class AppController extends Controller
{
    ...
    public function verificarInscripcion(int|string $cursoId): bool
    {
        ...
        // El docente instructor del curso (rol = 2) también tiene acceso automático
        if ($usuario->rol == 2) {
            $cursosTable = $this->fetchTable('Cursos');
            $curso = $cursosTable->find()
                ->select(['id', 'usuario_id'])
                ->where(['Cursos.id' => (int)$cursoId])
                ->first();

            if ($curso && (int)$curso->usuario_id === (int)$usuario->id) {
                return true;
            }
        }
    ...
    public function verificarInscripcionModulo(int|string $moduloId): bool
    {
        ...
        // El docente se valida contra el curso dueño del módulo vía verificarInscripcion()
    ...
    public function verificarInscripcionLeccion(int|string $leccionId): bool
    {
        ...
        // El docente se valida contra el curso dueño de la lección vía verificarInscripcion()
