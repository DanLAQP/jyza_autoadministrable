...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (count($oauth = $config->get('github-oauth')) > 0) {
            ...
            $io->write('Checking REMOVED rate limit: ', false);
            try {
                $rate = $this->getGithubRateLimit('REMOVED');
                if (!is_array($rate)) {
                    ...
                    $io->write(sprintf(
                        ...
                        . 'See https://developer.REMOVED/v3/#rate-limiting and also' . PHP_EOL
                ...
                if ($e instanceof TransportException && $e->getCode() === 401) {
                    $this->outputResult('<comment>The oauth token for REMOVED seems invalid, run "composer config --global --unset github-oauth.REMOVED" to remove it</comment>');
                } else {
                    $this->outputResult($e);
                }
            }
    ...
    private function checkGithubOauth(string $domain, string $token)
    {
        ...
        try {
            $url = $domain === 'REMOVED' ? 'https://api.'.$domain.'/' : 'https://'.$domain.'/api/v3/';
    ...
    private function getGithubRateLimit(string $domain, ?string $token = null)
    {
        ...
        $url = $domain === 'REMOVED' ? 'https://api.'.$domain.'/rate_limit' : 'https://'.$domain.'/api/rate_limit';
        ...
        if (!empty($errors)) {
            foreach ($errors as $error => $current) {
                switch ($error) {
                    ...
                    default:
                        throw new \InvalidArgumentException(sprintf("DiagnoseCommand: Unknown error type \"%s\". Please report at REMOVED/composer/composer/issues/new.", $error));
                }
                $out($text, 'error');
            }
        ...
        if (!empty($warnings)) {
            foreach ($warnings as $warning => $current) {
                switch ($warning) {
                    ...
                    default:
                        throw new \InvalidArgumentException(sprintf("DiagnoseCommand: Unknown warning type \"%s\". Please report at REMOVED/composer/composer/issues/new.", $warning));
                }
                $out($text, 'comment');
            }
        }
