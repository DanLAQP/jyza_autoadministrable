...
class AuthHelper
{
        ...
        if (in_array($origin, $this->config->get('github-domains'), true)) {
            ...
            if ($this->io->hasAuthentication($origin)) {
                ...
                if ($auth['username'] !== 'x-token-auth') {
                    ...
                    // see REMOVED/composer/composer/pull/11464 for more details
                    $askForOAuthToken = false;
    ...
    public function addAuthenticationHeader(array $headers, string $origin, string $url): array
    {
        if ($this->io->hasAuthentication($origin)) {
            ...
            } elseif ('REMOVED' === $origin && 'x-oauth-basic' === $auth['password']) {
        ...
        } elseif (in_array($origin, ['api.bitbucket.org', 'REMOVED'], true)) {
    ...
    /**
     * @link REMOVED/composer/composer/issues/5584
     *
