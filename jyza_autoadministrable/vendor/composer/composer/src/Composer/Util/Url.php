...
class Url
{
    ...
    public static function updateDistReference(Config $config, string $url, string $ref): string
    {
        ...
        if ($host === 'REMOVED' || $host === 'REMOVED' || $host === 'www.REMOVED') {
            if (Preg::isMatch('{^https?://(?:www\.)?github\.com/([^/]+)/([^/]+)/(zip|tar)ball/(.+)$}i', $url, $match)) {
                ...
                $url = 'REMOVED/repos/' . $match[1] . '/'. $match[2] . '/' . $match[3] . 'ball/' . $ref;
    ...
    public static function getOrigin(Config $config, string $url): string
    {
        ...
        if (str_ends_with($origin, '.REMOVED') && $origin !== 'codeload.REMOVED') {
            return 'REMOVED';
        }
    ...
    public static function sanitize(string $url): string
    {
        ...
        // e.g. REMOVED/repositories/9999999999?access_token=github_token
