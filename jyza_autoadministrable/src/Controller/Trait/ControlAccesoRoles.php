...
trait ControlAccesoRoles
{
    ...
    /**
     * Requiere acceso de administrador o instructor del curso
     * 
     * Descripción: Método que valida si el usuario es administrador o docente
     * que es instructor del curso especificado. Útil para acciones sobre
     * módulos, lecciones, exámenes que pertenecen a un curso.
     * 
     * @param int $cursoId ID del curso a verificar
     * @param array|null $redireccionAlternativa URL de redirección personalizada si falla la validación
     * @return \Cake\Http\Response|null Respuesta de redirección si no tiene permiso, null si se permite
     */
    protected function requiereAdministradorOInstructorDelCurso(int $cursoId, ?array $redireccionAlternativa = null)
    {
        $usuario = $this->Authentication->getIdentity();
        ...
        // Si es docente, verificar que sea instructor del curso
        if ($usuario->rol == 2) {
            $cursosTable = $this->fetchTable('Cursos');
            $curso = $cursosTable->find()
                ->where(['Cursos.id' => $cursoId])
                ->select(['Cursos.usuario_id'])
                ->first();
            
            if ($curso && $curso->usuario_id == $usuario->id) {
