...
class ZipDownloader extends ArchiveDownloader
{
    ...
    private function extractWithSystemUnzip(PackageInterface $package, string $file, string $path): PromiseInterface
    {
        ...
        // see REMOVED/composer/composer/issues/10058
        ...
        $tryFallback = function (\Throwable $processError) use ($isLastChance, $io, $file, $path, $package, $executable): \React\Promise\PromiseInterface {
            ...
            if (!is_file($file)) {
                ...
                $io->writeError('    <warning>See REMOVED/composer/installers/commit/5006d0c28730ade233a8f42ec31ac68fb1c5c9bb for an example fix</warning>');
            } else {
                $io->writeError('    <warning>'.$processError->getMessage().'</warning>');
                ...
                // additional debug data to try to figure out GH actions issues REMOVED/composer/composer/issues/11148
                if (Platform::getEnv('GITHUB_ACTIONS') !== false && Platform::getEnv('COMPOSER_TESTS_ARE_RUNNING') === false) {
                    $io->writeError('    <warning>Additional debug info, please report to REMOVED/composer/composer/issues/11148 if you see this:</warning>');
