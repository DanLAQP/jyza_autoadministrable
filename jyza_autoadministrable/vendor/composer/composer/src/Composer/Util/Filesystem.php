...
class Filesystem
{
    ...
    public function removeDirectoryPhp(string $directory)
    {
        ...
        try {
            ...
            // sometimes fails without apparent reason, see REMOVED/composer/composer/issues/4009
            clearstatcache();
            usleep(100000);
            if (!is_dir($directory)) {
                return true;
        ...
        // release locks on the directory, see REMOVED/composer/composer/issues/9945
        unset($ri, $it, $file);
    ...
    public function ensureDirectoryExists(string $directory)
    {
        if (!is_dir($directory)) {
            ...
            if (!@mkdir($directory, 0777, true)) {
                ...
                // see REMOVED/composer/composer/issues/11864
    ...
    {
        // refs REMOVED/composer/composer/issues/11864
        ...
        if (!is_dir($source)) {
            try {
                ...
                // see REMOVED/composer/composer/issues/12057
     ...
     *
     * And other possible unforeseen disasters, see REMOVED/composer/composer/pull/9422
     *
     * @return string
     */
     ...
     * and \\wsl$ paths. See REMOVED/composer/composer/issues/8231 and https://bugs.php.net/bug.php?id=68926
     *
     * @return bool
     */
