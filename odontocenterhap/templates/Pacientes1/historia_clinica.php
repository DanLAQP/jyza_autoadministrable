...
<div class="Fecha">FECHA: <?= h($pacientes1->paciente->created->format('Y-m-d')) ?></div>
...
<table style="width: 100%; border-collapse: collapse;">
    ...
    <tr>
        <td style="width: 80%;">
            <span class="label-text">Nombre y Apellido:</span> <?= h($pacientes1->nombre) ?> <?= h($pacientes1->apellido) ?>
        </td>
        <td style="width: 5%;"></td>
        <td style="width: 15%;">
            <span class="label-text">DNI:</span> <?= h($pacientes1->paciente->dni) ?>
        </td>
    </tr>
    <!-- Segunda Fila: Edad, Sexo y RUC -->
    <tr>
        <td style="width: 30%;">
            <span class="label-text">Edad:</span> <?= h($pacientes1->paciente->edad) ?>
        </td>
        
        <td style="width: 40%;">
            <span class="label-text">Sexo:</span> <?= h($pacientes1->paciente->sexo) ?>
        </td>
        <td style="width: 30%;">
            <span class="label-text">Ruc:</span> <?= h($pacientes1->paciente->sexo) ?>
        </td>
    </tr>
    <!-- tercera fila -->
    <tr>
        <td style="width: 30%; ">
            <span class="label-text">Fecha de Nacimiento:</span> <?= h($pacientes1->paciente->fecha_nacimiento) ?>
        </td>
    ...
    <?php if (!empty($paciente->pasaporte)): ?>
        <tr>
            <td style="width: 100%;">
                <span class="label-text">Número de Pasaporte:</span> <?= h($pacientes1->paciente->pasaporte) ?>
            </td>   
        </tr>
    <?php endif; ?>
    ...
    <?php if (!empty($paciente->carnet_extranjeria)): ?>
        <tr>
            <td style="width: 100%;">
                <span class="label-text">Carnet de extranjería:</span> <?= h($pacientes1->paciente->carnet_extranjeria) ?>
            </td>
        </tr>
    <?php endif; ?>
<?php endif; ?>
    <tr>
        <td style="width: 100%;">
            <span class="label-text">Apoderado - Titular:</span> <?= h($pacientes1->paciente->nombre_apoderado) ?>
        </td>   
    </tr>
    <tr>
        <td style="width: 100%;">
            <span class="label-text">Parentesco:</span> <?= h($pacientes1->paciente->parentesco_apoderado) ?>
        </td>
    </tr>
    
    <tr>
        <td style="width: 60%;">
            <span class="label-text">Dirección:</span> <?= h($pacientes1->paciente->direccion) ?>
        </td>
    </tr>
    <tr>
        <td style="width: 60%; ">
            <span class="label-text">Distrito:</span> <?= h($pacientes1->paciente->distrito) ?>
        </td>
        <td style="width: 40%;">
            <span class="label-text">Código Postal:</span> <?= h($pacientes1->paciente->codigo_postal) ?>
        </td>
    </tr>
    <tr>
    <td style="width: 40%; ">
        <span class="label-text">E-mail:</span> <?= h($pacientes1->paciente->email) ?>
    </td>
        <td style="width: 30%;">
            <span class="label-text">Teléfono:</span> <?= h($pacientes1->paciente->telefono_oficina) ?>
        </td>
        <td style="width: 30%; ">
            <span class="label-text">Celular:</span> <?= h($pacientes1->telefono_celular) ?>
        </td>
    </tr>
    <tr>
        <td style="width: 100%;">
            <span class="label-text">Centro de trabajo:</span> <?= h($pacientes1->paciente->centro_trabajo) ?>
        </td>
    </tr>
        <tr>
            <td style="width: 100%;">
                <span class="label-text">Centro de estudios:</span> <?= h($pacientes1->paciente->centro_estudio) ?>
            </td>
        </tr>
    <tr>
        <td style="width: 100%;">
            <span class="label-text">Recomendado por:</span> <?= h($pacientes1->paciente->recomendacion) ?>
        </td>
    </tr>
    ...
    <td style="width: 55%;"></td>
        ...
        <?php endforeach; ?>
    <?php else : ?>
        <p>No hay antecedentes medicos registrados.</p>
    <?php endif; ?>
