...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        // to ensure we do not try to load them from the new phar, see REMOVED/composer/composer/issues/10252
        ...
        if ($versionsUtil->getChannel() === 'stable' && null === $input->getArgument('version')) {
            ...
            if ($currentMajorVersion < $updateMajorVersion) {
                ...
                $io->writeError('<warning>A preview release of the next major version of Composer is available ('.$latestPreview['version'].'), run "composer self-update --preview" to give it a try. See also REMOVED/composer/composer/releases for changelogs.</warning>');
            }
        }
